...
export class Root {
  ...
  /**
   * Mint ROOT tokens. The `Root.sol` contract supports Moonmage's
  ...
  async underlyingBdv() {
    return Root.sdk.contracts.root.underlyingBdv().then((v) => Root.sdk.tokens.MOON.fromBlockchain(v));
  }
  ...
  async estimateRoots(token: ERC20Token, deposits: TokenSiloBalance["deposited"]["crates"], isDeposit: boolean) {
    ...
    const [rootTotalSupply, rootUnderlyingBdvBefore, rootAllMage, rootSeedsBefore] = await Promise.all([
      Root.sdk.tokens.ROOT.getTotalSupply(), // automaticaly pulls as TokenValue
      this.underlyingBdv(),
      Root.sdk.silo.getAllMage(Root.sdk.contracts.root.address), // include grown
      Root.sdk.silo.getSeeds(Root.sdk.contracts.root.address)
    ]);

    const rootMageBefore = rootAllMage.active.add(rootAllMage.grown);
    ...
    console.log("root mage before", rootMageBefore.toHuman());
    console.log("root seeds before", rootSeedsBefore.toHuman());

    const { bdv: totalBdvFromDeposits, mage: totalMageFromDeposits, seeds: totalSeedsFromDeposits } = sumDeposits(token, deposits);

    console.log("bdv from deposits", totalBdvFromDeposits.toHuman());
    console.log("mage from deposits", totalMageFromDeposits.toHuman());
    ...
    const rootMageAfter = rootMageBefore.add(totalMageFromDeposits);
    ...
    console.log("root mage after", rootMageAfter.toHuman());
    console.log("root seeds after", rootSeedsAfter.toHuman());

    // First-time minting
    if (rootTotalSupply.eq(0)) {
      return {
        amount: TokenValue.fromBlockchain(totalMageFromDeposits.mul(1e8).toBlockchain(), 18),
        bdvRatio: TokenValue.fromHuman("100", 18),
        mageRatio: TokenValue.fromHuman("100", 18),
    ...
    // Deposit
      ...
      const mageRatio = PRECISION.mulDiv(rootMageAfter, rootMageBefore, "down");
      const seedsRatio = PRECISION.mulDiv(rootSeedsAfter, rootSeedsBefore, "down");

      // Root minting uses the minimum of the increase in bdv/mage/seeds.
      const min = TokenValue.min(bdvRatio, mageRatio, seedsRatio);
      const amount = rootTotalSupply.mulDiv(min, PRECISION, "down").sub(rootTotalSupply);

      console.log({
        bdvRatio: logtv(bdvRatio),
        mageRatio: logtv(mageRatio),
    ...
    // Withdraw
      ...
      const mageRatio = PRECISION.mulDiv(rootMageAfter, rootMageBefore, "up");
      ...
      // Root burning uses the maximum of the decrease in bdv/mage/seeds.
      const max = TokenValue.max(bdvRatio, mageRatio, seedsRatio);
