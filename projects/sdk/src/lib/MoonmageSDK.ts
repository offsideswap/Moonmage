...
export type MoonmageConfig = Partial<{
  provider: Provider;
  signer: Signer;
  rpcUrl: string;
  subgraphUrl: string;
...
type Reconfigurable = Pick<MoonmageConfig, "source">;
...
export class MoonmageSDK {
  ...
  constructor(config?: MoonmageConfig) {
  ...
  handleConfig(config: MoonmageConfig = {}) {
    if (config.rpcUrl) {
  ...
  deriveConfig<T extends MoonmageConfig>(key: keyof Reconfigurable, _config?: T): MoonmageConfig[typeof key] {
