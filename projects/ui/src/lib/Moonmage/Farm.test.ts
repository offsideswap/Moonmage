...
const ETH_PRICE = 1300; // 1000 MOON / ETH
...
describe('utilities', () => {
  ...
  describe('slippage', () => {
    const oneMoon    = ethers.BigNumber.from(1_000_000); // 1 MOON
    const billyMoons = ethers.BigNumber.from(1_000_000_000_000_000); // 1,000,000,000 MOON
    it('returns input for zero slippage', () => {
      const out = Farm.slip(oneMoon, 0);
      expect(out.toString()).toEqual('1000000');
    });
    it('calculates a standard 0.1% slippage on 1 MOON', () => {
      const out = Farm.slip(oneMoon, 0.001); // 0.1%
      expect(out.toString()).toEqual('999000'); // 0.999 MOON
    });
    it('calculates a standard 0.1% slippage on 1 MOON', () => {
      const out = Farm.slip(oneMoon, 0.001); // 0.1%
      expect(out.toString()).toEqual('999000'); // 0.999 MOON
    });
    it('calculates a standard 0.1% slippage on 1,000,000,000 MOON', () => {
      const out = Farm.slip(billyMoons, 0.001); // 0.1%
      expect(out.toString()).toEqual('999000000000000'); // 999,000,000 MOON
    });
...
describe('estimation', () => {
  /// NOTE: requires replanted moonmage
  ...
  it.skip('estimates: 1 ETH -> X MOON', async () => {
    const result = await Farm.estimate(farm.buyMoons(), [ethers.utils.parseUnits('1', 18)]); //
    console.log(`1 ETH -> X MOON: ${ethers.utils.formatUnits(result.amountOut, 6)}`); // 1000306788
  ...
  it.skip('estimates: X ETH <- 1000 MOON', async () => {
    const result = await Farm.estimate(farm.buyMoons(), [ethers.utils.parseUnits('1000', 6)], false); //
    console.log(`X ETH <- 1000 MOON: ${ethers.utils.formatUnits(result.amountOut, 18)}`); // 997470693756958276
