...
import {
  ...
  RemoveDepositsEvent,
  PodListingCancelledEvent, PodListingCreatedEvent, PodListingFilledEvent, PodOrderCancelledEvent, PodOrderCreatedEvent, PodOrderFilledEvent } from '~/generated/protocol/abi/Moonmage';
...
export default class EventProcessor {
  ...
  AddDeposit(event: Simplify<AddDepositEvent>) {
    ...
    const bdv       = tokenBN(event.args.bdv, Moon);
  ...
  PodListingCreated(event: Simplify<PodListingCreatedEvent>) {
    ...
    //   filledAmount:     BN(0),    // 
    //   minFillAmount:    tokenBN(event.args.minFillAmount || 0, MOON[1]),
  ...
  PodOrderCreated(event: Simplify<PodOrderCreatedEvent>) {
    ...
    //   podAmount:      tokenBN(event.args.amount, MOON[1]),
    //   pricePerPod:      tokenBN(event.args.pricePerPod, MOON[1]),
    //   podAmountRemaining:  tokenBN(event.args.amount, MOON[1]),
