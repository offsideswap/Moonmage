...
import { Moonmage__factory, Curve3Pool__factory, CurveTriCrypto2Pool__factory, CurveRegistry__factory, CurveMetaFactory__factory, CurveCryptoFactory__factory, CurveMetaPool__factory, CurvePlainPool__factory, CurveZap__factory } from '~/generated';
import { getChainConstant } from '~/util/Chain';
import { MOONMAGE_ADDRESSES, MOON_CRV3_ADDRESSES, CRYPTO_FACTORY_ADDRESSES, CURVE_ZAP_ADDRESSES, META_FACTORY_ADDRESSES, POOL3_ADDRESSES, POOL_REGISTRY_ADDRESSES, TRICRYPTO2_ADDRESSES } from '~/constants';
import { MOON, USDT, WETH } from '~/constants/tokens';
 ...
 * 1567.31 USDT     1464.77 MOON      get_dy_underlying     i=3, j=0     
 * -------------------------------------------------------------------------
 * amountOut = 1464.77 MOON
 * 
 * Backward calcuation:
 ...
 *  - I want to buy Pods from a Pod Listing. It's going to cost me 1,000 Moons but I only have ETH.
 *    How much ETH should I input to buy 1,000 Moons?
 *  - I want to Sow; there is 100 Soil available (so I can only Sow 100 Moons). How much ETH should I swap
 *    to MOON to Sow all 100 Soil?
 * 
 * ? ETH <- 1000 MOON
 * amountInStep  |  amountOutStep  |  lookup function    |  params
 * -------------------------------------------------------------------------
 * 1000 MOON        1068.44 USDT      get_dy_underlying     i=0, j=3
...
const getContracts = (provider: ethers.providers.BaseProvider) => {
  ...
  const MOONMAGE       = getChainConstant(MOONMAGE_ADDRESSES, chainId);
  const POOL3           = getChainConstant(POOL3_ADDRESSES, chainId);
  const TRICRYPTO2      = getChainConstant(TRICRYPTO2_ADDRESSES, chainId);
  const MOONCRV3        = getChainConstant(MOON_CRV3_ADDRESSES, chainId);
  ...
  const moonCrv3        = CurveMetaPool__factory.connect(MOONCRV3, provider);
  ...
  return {
    moonmage: Moonmage__factory.connect(MOONMAGE, provider),
    curve: {
      // Pools
      pools: {
        pool3,
...
export default class Farm {
  ...
  buyMoons = (
    ...
    // USDT -> MOON via moon3crv exchange_underlying
    this.exchangeUnderlying(
      this.contracts.curve.pools.moonCrv3.address,
      getChainConstant(USDT, this.provider.network.chainId).address,
      getChainConstant(MOON, this.provider.network.chainId).address,
    ),
  ]
  ...
  pair = {
    WETH_MOON: (
      ...
      // default: WETH -> MOON; flip if input is MOON
      ...
      const Moon = getChainConstant(MOON, this.provider.network.chainId).address;
  ...
  wrapEth = (
    ...
    return {
      ...
      encode: (_: ethers.BigNumber) => (
        this.contracts.moonmage.interface.encodeFunctionData('wrapEth', [
          _amountInStep,        // ignore minAmountOut since there is no slippage
          _toMode,              //
        ])
      ),
      decode: (data: string) => this.contracts.moonmage.interface.decodeFunctionData('wrapEth', data),
    };
  };

  unwrapEth = (
    /// Default to EXTERNAL because Moonmage can't store INTERNAL ETH.
    ...
    return {
      ...
      amountOut: _amountInStep, // amountInStep should be an amount of ETH.
      encode: (_: ethers.BigNumber) => (
        this.contracts.moonmage.interface.encodeFunctionData('unwrapEth', [
          _amountInStep,        // ignore minAmountOut since there is no slippage
          _fromMode,            //
        ])
      ),
      decode: (data: string) => this.contracts.moonmage.interface.decodeFunctionData('unwrapEth', data),
    };
  };
  ...
  transferToken = (
    ...
    return {
      ...
      encode: (_: ethers.BigNumber) => (
        this.contracts.moonmage.interface.encodeFunctionData('transferToken', [
          _tokenIn,      //
          _recipient,    //
          _amountInStep, // ignore minAmountOut since there is no slippage
          _fromMode,     //
      ...
      decode: (data: string) => this.contracts.moonmage.interface.decodeFunctionData('transferToken', data),
    };
  };
  ...
  exchange = (
    ...
    return {
      ...
      encode: (minAmountOut: ethers.BigNumber) => (
        this.contracts.moonmage.interface.encodeFunctionData('exchange', [
          _pool,
          _registry,
          tokenIn,
          tokenOut,
      ...
      decode: (data: string) => this.contracts.moonmage.interface.decodeFunctionData('exchange', data),
      data: {
        pool: _pool,
        registry: _registry,
        tokenIn,
  ...
  exchangeUnderlying = (
    ...
    return {
      ...
      amountOut,
      encode: (minAmountOut: ethers.BigNumber) => (
        this.contracts.moonmage.interface.encodeFunctionData('exchangeUnderlying', [
          _pool,
          tokenIn,
          tokenOut,
          _amountInStep,
      ...
      decode: (data: string) => this.contracts.moonmage.interface.decodeFunctionData('exchangeUnderlying', data),
      data: {
        pool: _pool,
        tokenIn,
        tokenOut,
  ...
  addLiquidity(
    ...
    return async (_amountInStep: ethers.BigNumber) => {
      ...
      return {
        ...
        amountOut,
        encode: (minAmountOut: ethers.BigNumber) => (
          this.contracts.moonmage.interface.encodeFunctionData('addLiquidity', [
            _pool,
            _registry,
        ...
        decode: (data: string) => this.contracts.moonmage.interface.decodeFunctionData('addLiquidity', data),
        data: {
          pool: _pool,
          registry: _registry,
          fromMode: _fromMode,
  ...
  removeLiquidityOneToken(
    ...
    return async (_amountInStep: ethers.BigNumber) => {
      ...
      return {
        ...
        amountOut,
        encode: (minAmountOut: ethers.BigNumber) => (
          this.contracts.moonmage.interface.encodeFunctionData('removeLiquidityOneToken', [
            _pool,
            _registry,
            _tokenOut,
            _amountInStep,
        ...
        decode: (data: string) => this.contracts.moonmage.interface.decodeFunctionData('removeLiquidityOneToken', data),
        data: {}
      };
    };
  }
