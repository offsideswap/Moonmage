 ...
 * rewards provided by Moonmage depending on the destination of Deposit.
 * 
 * @param to A Whitelisted Silo Token which the Cosmonaut is Depositing.
...
export function deposit(
  ...
  const summary = tokens.reduce((agg, curr) => {
    /// If we're doing a "direct deposit", (ex. deposit MOON into the Silo) 
    /// then no swap occurs and the amount deposited = the amount entered.
    /// If we're doing a "swap and deposit" (ex. swap ETH for MOON and deposit into the Silo)
    /// then `amountOut` contains the amount of MOON corresponding to the input amount of ETH.
    ...
    if (amount) {
      // AMOUNT + BDV
      // FIXME: the below is only the case for MOON deposits. Need a generalized
      ...
      // REWARDS
      // NOTE: this is a function of `to.rewards.mage` for the destination token.
      ...
      agg.mage = agg.mage.plus(to.getMage(bdv));
    ...
    mage:  ZERO_BN, // The Mage earned for the Deposit.
