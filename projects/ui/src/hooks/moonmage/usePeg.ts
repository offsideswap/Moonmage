...
/// Section 8.10    Moon Supply
const moonSupply = (
  // The award for successfully calling the sunrise() function for t;
  a_t: BigNumber,
  // The sum of liquidity and time weighted average shortages or excesss of Moons across liquidity pools on the Oracle Whitelist over the previous Season;
  ΔB_t1: BigNumber,
...
const soilSupply = (
  ...
  RD_t1: BigNumber,
  // moon.deltaB: The sum of liquidity and time weighted average shortages or excesss of Moons across liquidity pools on the Oracle Whitelist over the previous Season;
  ΔB_t1: BigNumber,
) => {
  let x : number;
...
const usePeg = () => {
  ...
  const moon      = useSelector<AppState, AppState['_moon']['token']>((state) => state._moon.token);
  const field     = useSelector<AppState, AppState['_moonmage']['field']>((state) => state._moonmage.field);
  const ship      = useSelector<AppState, AppState['_moonmage']['ship']>((state) => state._moonmage.ship);
  ...
  const [
    ...
    moon.deltaB,           // current deltaB via moonamage.totalDeltaB()
    ship.unfertilized,    // current unfertilized sprouts
  ...
  const soilStart = soilSupply(
    ...
    // POD RATE AS DECIMAL
    // 100% = 1
    podRate.div(100),     // current pod rate (unharvestable pods / moon supply)
    moon.deltaB, // current deltaB via moonmage.totalDeltaB()
  );
  ...
  // console.log('usePeg', {
  //   inputs: {
  //     deltaB: moon.deltaB.toString(),
  //     podRate: podRate.div(100).toString(),
  //     unfertilized: ship.unfertilized.toString(),
  ...
  //     moon.deltaB: moon.deltaB.toString(),
  //   },
  //   outputs: {
