      ...
      const parseEvent = (e: typeof events[number]) => {
        switch (e.__typename) {
          case 'PodOrderCreated': {
            const pricePerPod = toTokenUnitsBN(e.pricePerPod, MOON[1].decimals);
            const amount = toTokenUnitsBN(e.amount, MOON[1].decimals);
            const placeInLine = toTokenUnitsBN(e.maxPlaceInLine, MOON[1].decimals);        
            // HOTFIX: amountPods is using the legacy moon amount format for these events
            const amountPods = amount;
            const amountMoons = amount.multipliedBy(pricePerPod);
            return <MarketEvent>{
              id: 'unknown',
              eventId: e.id,
              hash: e.hash,
              ...
              amountMoons: amountMoons,
              amountUSD: getUSD(MOON[1], amountMoons),
          ...
          case 'PodOrderCancelled': {
            ...
            const podAmount = toTokenUnitsBN(podOrder.podAmount || 0, MOON[1].decimals);
            const pricePerPod = toTokenUnitsBN(new BigNumber(podOrder.pricePerPod || 0), MOON[1].decimals);
            const totalMoons = podAmount && pricePerPod
            ...
            return <MarketEvent>{
              ...
              amountPods: toTokenUnitsBN(podOrder?.podAmount, MOON[1].decimals),
              placeInLine: toTokenUnitsBN(podOrder?.maxPlaceInLine, MOON[1].decimals),
              pricePerPod: toTokenUnitsBN(new BigNumber(podOrder?.pricePerPod || 0), MOON[1].decimals),
              amountMoons: totalMoons,
              amountUSD: totalMoons ? getUSD(MOON[1], totalMoons) : undefined,
              createdAt: e.createdAt,
            };
          }
          case 'PodOrderFilled': {
            ...
            const pricePerPod = toTokenUnitsBN(new BigNumber(podOrder.pricePerPod || 0), MOON[1].decimals);
            const podAmountFilled = toTokenUnitsBN(podOrder.podAmountFilled, MOON[1].decimals);
            const totalMoons =  getUSD(MOON[1], podAmountFilled.multipliedBy(pricePerPod));
            return <MarketEvent> {
              id: podOrder.id,
              eventId: e.id,
              hash: e.hash,
              ...
              placeInLine: toTokenUnitsBN(new BigNumber(e.index), MOON[1].decimals).minus(harvestableIndex),
              pricePerPod: pricePerPod,
              amountMoons: totalMoons,
              amountUSD: getUSD(MOON[1], totalMoons),
              createdAt: e.createdAt,
            };
          }
          case 'PodListingCreated': {
            const numPods = toTokenUnitsBN(e.amount, MOON[1].decimals);
            const pricePerPod = toTokenUnitsBN(e.pricePerPod, MOON[1].decimals);
            const totalMoons = numPods.multipliedBy(pricePerPod);
            return <MarketEvent> {
              ...
              action: 'create' as const,
              amountPods: numPods,
              placeInLine: toTokenUnitsBN(e.index, MOON[1].decimals).minus(harvestableIndex),
              pricePerPod: pricePerPod,
              amountMoons: totalMoons,
              amountUSD: getUSD(MOON[1], totalMoons),
              createdAt: e.createdAt,
          ...
          case 'PodListingCancelled': {
            ...
            const numPods = toTokenUnitsBN(podListing.amount, MOON[1].decimals);
            const pricePerPod = toTokenUnitsBN(new BigNumber(podListing.pricePerPod || 0), MOON[1].decimals);
            const totalMoons = numPods.multipliedBy(pricePerPod);
            ...
            return <MarketEvent> {
              ...
              action: 'cancel' as const,
              amountPods: numPods,
              placeInLine: toTokenUnitsBN(podListing?.index, MOON[1].decimals).minus(harvestableIndex),
              pricePerPod: pricePerPod,
              amountMoons: totalMoons,
              amountUSD: getUSD(MOON[1], totalMoons),
              createdAt: e.createdAt,
          ...
          case 'PodListingFilled': {
            ...
            const numPodsFilled = toTokenUnitsBN(podListing?.filledAmount, MOON[1].decimals);
            const pricePerPod = toTokenUnitsBN(new BigNumber(podListing?.pricePerPod || 0), MOON[1].decimals);
            const totalMoons = numPodsFilled.multipliedBy(pricePerPod);
            return <MarketEvent> {
              id: e.historyID.split('-')[1],
              eventId: e.id,
              hash: e.hash,
