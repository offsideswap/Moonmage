...
import { updateTokenPrices } from '~/state/moonmage/tokenPrices/actions';
...
export default function useDataFeedTokenPrices() {
  const tokenPriceMap = useSelector<AppState, AppState['_moonmage']['tokenPrices']>((state) => state._moonmage.tokenPrices);
  ...
  const fetch = useCallback(async () => {
    ...
    console.debug('[moonmage/tokenPrices/useCrvUnderlylingPrices] FETCH');
    ...
    console.debug(`[moonmage/tokenPrices/useCrvUnderlyingPrices] RESULT: ${priceDataCache}`);
