...
import usePodListings from '~/hooks/moonmage/usePodListings';
import { castPodListing, castPodOrder, PodListing, PodOrder } from '~/state/cosmomage/market';
...
const useMarketData = () => {
  ...
  orders = orders?.filter((order) => order.moonAmountRemaining.gt(MIN_POD_AMOUNT));
