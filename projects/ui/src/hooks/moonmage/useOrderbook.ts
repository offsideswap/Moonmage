...
export type PriceBucket = {
  depth: {
    ...
    moon: BigNumber; // depth of moons at price point
  };
  placeInLine: {
    buy: {
...
const handleBucketOrders = (orders: PodOrder[]) => orders.reduce((prev, order) => {
    ...
    const moonAmount = order.podAmountRemaining.times(order.pricePerPod);

    // add to the depth of moons at the price point
    bucket.depth.moon = bucket.depth.moon.plus(moonAmount);
...
export default function useOrderbook() {
  ...
  const values = useMemo(() => {
    ...
    return basePriceKeys.reduce<PriceBuckets>((prev, curr) => {
      ...
      if (orderBuckets[curr]?.depth.moon.gt(0)) {
        bucket.depth.moon = orderBuckets[curr].depth.moon;
      }
    ...
    ({
      ...
      const reduced = Object.entries(priceBuckets).reduce(
        (prev, [price, data]) => {
          ...
          // transfer depth data
          bucket.depth.moon = bucket.depth.moon.plus(data.depth.moon);
