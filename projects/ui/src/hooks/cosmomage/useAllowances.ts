...
export default function useAllowances(
  ...
  const allowances = useSelector<AppState, AppState['_cosmomage']['allowances']>((state) => state._cosmomage.allowances);
  const account = useAccount();
  const [fetchAllowances] = useFetchCosmonautAllowances();
