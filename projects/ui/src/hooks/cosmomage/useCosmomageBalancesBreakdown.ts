...
import useSiloTokenToFiat from '../moonmage/useSiloTokenToFiat';
...
export const STATE_CONFIG = {
  ...
  farm: [
    'Farm',
    '#F2E797',
    'Assets stored in Moonmage. Farm assets can be used in transactions on the Farm.'
  ],
  circulating: [
    'Circulating',
    MoonmagePalette.lightBlue, 
    'Moonmage assets in your wallet.'
  ],
} as const;
...
export type SiloStateBreakdown = {
   ...
   * Ex. I have a Moon deposit: 0xMOON => { amount: 100, value: 101 } if 1 MOON = $1.01
   */
 ...
 */
export default function useCosmonautBalancesBreakdown() {
  /// Constants
  const whitelist = useWhitelist();
  const whitelistAddrs = useMemo(() => Object.keys(whitelist), [whitelist]);

  /// Balances
  const siloBalances  = useSelector<AppState, AppState['_cosmomage']['silo']['balances']>((state) => state._cosmomage.silo.balances);
  const tokenBalances = useSelector<AppState, AppState['_cosmomage']['balances']>((state) => state._cosmomage.balances);
