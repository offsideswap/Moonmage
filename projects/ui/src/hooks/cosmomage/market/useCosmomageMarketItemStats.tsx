...
const isListing = (i: CosmomageStationOrder) => i.type === 'listing' && i.source;
const isOrder = (i: CosmomageStationOrder) => i.type === 'order' && i.source;

export default function useCosmomageStationItemStats(
  ...
  const data = useMemo(() => {
    ...
    items.push({
      label: 'TOTAL',
      info: `${displayFullBN(item.amountMoons, 2)} MOON`,
    });
    items.push({
      label: 'STATUS',
      info: item.status,
