...
const columns = [
  ...
  MarketColumns.HistoryItem.amountMoons(1.3),
  MarketColumns.HistoryItem.fillPct(0.6),
  MarketColumns.Shared.expiry(0.5),
  MarketColumns.HistoryItem.status(0.6, 'right'),
];
/**
 * Displays a table of a Cosmonaut's outstanding Listings and Orders.
 */
const CosmonautOrders: React.FC<{
  data: CosmomageStationOrder[] | undefined;
  initializing: boolean;
}> = ({ data, initializing }) => {
  const [open, setOpen] = useState(false);
  const [showModeDialog, setShowModeDialog] = useState(false);
  const [activeItem, setActiveItem] = useState<CosmomageStationOrder | undefined>(undefined);
...
export default CosmonautOrders;
