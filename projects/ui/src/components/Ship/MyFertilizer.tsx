...
import FertilizerItem from '~/components/Ship/FertilizerItem';
import { MY_FERTILIZER } from '~/components/Ship/FertilizerItemTooltips';
...
import { FertilizerBalance } from '~/state/cosmomage/ship';
...
const MyFertilizer: FC<{}> = () => {
  ...
  const cosmomageShip = useSelector<AppState, AppState['_cosmomage']['ship']>((state) => state._cosmomage.ship);

  /// Helpers
  const [tab, handleChange] = useTabs();
  const pctRepaid = useCallback((balance: FertilizerBalance) => (
    MinBN(
      (moonmageShip.currentBpf.minus(balance.token.startBpf))
        .div(balance.token.id.minus(balance.token.startBpf)),
      ONE_BN
    )
  ), [moonmageShip.currentBpf]);

  const filteredBalances = useMemo(() => cosmomageShip.balances?.filter((balance) => {
    const pct = pctRepaid(balance);
    if (tab === TabState.ACTIVE && pct.gte(1)) return false;
    if (tab === TabState.USED && pct.lt(1)) return false;
    return true;
  }) || [], [cosmomageShip.balances, pctRepaid, tab]);
    ...
    <Card>
      ...
      <Stack sx={{ p: 2 }} gap={1}>
        ...
        <Stack gap={1}>
          ...
          >
            <Tooltip
              title="The number of Moons left to be earned from your Fertilizer. Sprouts become Rinsable on a pari passu basis."
              placement="bottom"
            >
            ...
            <Row alignItems="center" gap={0.2}>
              ...
              <Typography>
                {displayFullBN(
                  MaxBN(cosmomageShip.unfertilizedSprouts, ZERO_BN), SPROUTS.displayDecimals
                )}
              </Typography>
            </Row>
          </Row>
          ...
          >
            <Tooltip
              title="Sprouts that are redeemable for 1 Moon each. Rinsable Sprouts must be Rinsed in order to use them."
              placement="bottom"
            >
              <Typography variant="body1">
                Rinsable Sprouts&nbsp;
            ...
            <Row alignItems="center" gap={0.2}>
              ...
              <Typography>
                {displayFullBN(
                  MaxBN(cosmomageShip.fertilizedSprouts, ZERO_BN), RINSABLE_SPROUTS.displayDecimals
                )}
              </Typography>
            </Row>
          </Row>
