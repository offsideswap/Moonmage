...
import useGenerateChartSeries from '~/hooks/moonmage/useGenerateChartSeries';
import useSeasonsQuery from '~/hooks/moonmage/useSeasonsQuery';
...
const VolumeChart: FC<{ width?: number; height: number }> = ({
  ...
  const queryConfig = useMemo(() => ({ context: { subgraph: 'moon' } }), []);
  ...
  return (
    <>
      ...
      >
        <ChartInfoOverlay
          title="Volume"
          titleTooltip="The total volume in the MOON:3CRV pool in every Season."
          gap={0.25}
