...
const address  = MOONMAGE_PRICE_ADDRESSES[1]; // '0xBEA0000029AD1c77D3d5D23Ba2D8893dB9d1Efab';
const moon     = ERC20__factory.connect(address, provider);

/**
 * Load the latest Moon supply from Ethereum.
 */
const _handler: Handler = async () => {
  try {
    return {
      statusCode: 200,
      body: ethers.utils.formatUnits(await moon.totalSupply(), 6),
    };
  } catch (e) {
    console.error(e);
    return {
