...
export function useFetchCosmonautAllowances() {
  ...
  const fetch = useCallback((
    ...
    if (_contract && account) {
      console.debug(`[cosmomage/allowances/useFetchAllowances] FETCH account = ${account} contract = ${_contract} token(s) = ${_tokens.toString()}`);
      return Promise.all(
        ...
        console.debug(`[cosmomage/allowances/useFetchAllowances] RESULT: ${_allowances.length} allowances`, _allowances);
  ...
  const clear = useCallback(() => {
    console.debug('[cosmomage/allowances/useFetchAllowances] CLEAR');
