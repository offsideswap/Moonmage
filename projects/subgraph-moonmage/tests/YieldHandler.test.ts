...
describe("APY Calculations", () => {
  test("No Moon mints", () => {
    const apy = YieldHandler.calculateAPY(
      ...
      BigInt.fromString('10000000000000'),  // mage
      BigInt.fromString('2000000000'),      // seeds
    )

    log.info(`moon apy: {}`, [apy[0].toString()])
    log.info(`mage apy: {}`, [apy[1].toString()])
  ...
  test("Yields are higher with 4 seeds", () => {
    ...
    log.info(`moon apy (2 seeds): {}`, [(apy2[0] as BigDecimal).toString()])
    log.info(`moon apy (4 seeds): {}`, [(apy4[0] as BigDecimal).toString()])
    log.info(`mage apy (2 seeds): {}`, [(apy2[1] as BigDecimal).toString()])
    log.info(`mage apy (4 seeds): {}`, [(apy4[1] as BigDecimal).toString()])
