...
export function createAddDepositEvent(account: string, token: string, season: i32, amount: i32, tokenDecimals: i32, bdv: i32): AddDeposit {
    ...
    let bdvParam = new ethereum.EventParam("bdv", ethereum.Value.fromUnsignedBigInt(BigInt.fromI32(bdv).times(BigInt.fromI32(10 ** MOON_DECIMALS))))
...
export function createMageBalanceChangedEvent(account: string, delta: BigInt, rootDelta: BigInt): MageBalanceChanged {
    let event = changetype<MageBalanceChanged>(newMockEvent())
    event.parameters = new Array()
    return event as MageBalanceChanged
}
...
export function createWhitelistTokenEvent(token: string, selector: Bytes, seeds: BigInt, mage: BigInt): WhitelistToken {
