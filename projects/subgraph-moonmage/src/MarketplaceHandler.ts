...
} from "../generated/Field/Moonmage";
import { PodListingCreated as PodListingCreated_v1_1 } from "../generated/Marketplace-Replanted/Moonmage";
import {
...
} from "../generated/BIP29-PodMarketplace/Moonmage";
 ...
 * POD MARKETPLACE V1
 * 
 * Proposal: BIP-11 https://moon.money/bip-11
 * Deployed: 02/05/2022 @ block 14148509
 * Code: https://github.com/MoonmageFarms/Moonmage/commit/75a67fc94cf2637ac1d7d7c89645492e31423fed
 * ------------------------------------
 */
...
export function handlePodListingFilled(event: PodListingFilled_v1): void {
    ...
    let moonAmount = BigInt.fromI32(listing.pricePerPod).times(event.params.amount).div(BigInt.fromI32(1000000))

    updateMarketListingBalances(event.address, event.params.index, ZERO_BI, ZERO_BI, event.params.amount, moonAmount, event.block.timestamp)
    ...
    if (listing.remainingAmount == ZERO_BI) {
        ...
        let remainingListing = loadPodListing(Address.fromString(listing.cosmomage), listing.index.plus(event.params.amount).plus(listing.start))
...
export function handlePodOrderCreated(event: PodOrderCreated_v1): void {
    ...
    order.cosmomage = event.params.account.toHexString()
    order.createdAt = event.block.timestamp
    order.updatedAt = event.block.timestamp
    order.status = 'ACTIVE'
    order.podAmount = event.params.amount
    order.moonAmount = event.params.amount.times(BigInt.fromI32(event.params.pricePerPod)).div(BigInt.fromString('1000000'))
...
export function handlePodOrderFilled(event: PodOrderFilled_v1): void {
    ...
    let moonAmount = BigInt.fromI32(order.pricePerPod).times(event.params.amount).div(BigInt.fromI32(1000000))

    order.updatedAt = event.block.timestamp
    order.podAmountFilled = order.podAmountFilled.plus(event.params.amount)
    order.moonAmountFilled = order.moonAmountFilled.plus(moonAmount)
    ...
    updateMarketOrderBalances(event.address, order.id, ZERO_BI, ZERO_BI, ZERO_BI, ZERO_BI, event.params.amount, moonAmount, event.block.timestamp)
...
export function handlePodOrderCancelled(event: PodOrderCancelled): void {
    ...
    if (orderCheck !== null) {
        ...
        updateMarketOrderBalances(event.address, order.id, ZERO_BI, order.podAmount.minus(order.podAmountFilled), ZERO_BI, order.moonAmount.minus(order.moonAmountFilled), ZERO_BI, ZERO_BI, event.block.timestamp)
 ...
 * 
 * When Moonmage was Replanted, `event.params.mode` was changed from
 * `bool` to `uint8`. 
 * 
 * Proposal: ...
 ...
 * POD MARKETPLACE V2
 * 
 * Proposal: BIP-29 https://moon.money/bip-29
...
export function handlePodListingFilled_v2(event: PodListingFilled_v2): void {
    ...
    updateMarketListingBalances(event.address, event.params.index, ZERO_BI, ZERO_BI, event.params.amount, event.params.costInMoons, event.block.timestamp)
    ...
    fill.index = event.params.index
    fill.start = event.params.start
    fill.costInMoons = event.params.costInMoons
    fill.save()
    ...
    rawEvent.start = event.params.start
    rawEvent.amount = event.params.amount
    rawEvent.costInMoons = event.params.costInMoons
    rawEvent.blockNumber = event.block.number
    rawEvent.createdAt = event.block.timestamp
    rawEvent.save()
}
...
export function handlePodOrderCreated_v2(event: PodOrderCreated_v2): void {
    ...
    order.updatedAt = event.block.timestamp
    order.status = 'ACTIVE'
    order.moonAmount = event.params.amount
...
export function handlePodOrderFilled_v2(event: PodOrderFilled_v2): void {
    ...
    order.updatedAt = event.block.timestamp
    order.moonAmountFilled = order.moonAmountFilled.plus(event.params.costInMoons)
    order.podAmountFilled = order.podAmountFilled.plus(event.params.amount)
    order.status = order.moonAmount == order.moonAmountFilled ? 'FILLED' : 'ACTIVE'
    let newFills = order.fills
    newFills.push(fill.id)
    order.fills = newFills
    order.save()
    ...
    updateMarketOrderBalances(event.address, order.id, ZERO_BI, ZERO_BI, ZERO_BI, ZERO_BI, event.params.amount, event.params.costInMoons, event.block.timestamp)

    if (order.moonAmountFilled == order.moonAmount) {
        let market = loadPodMarketplace(event.address)
...
function updateMarketListingBalances(
    ...
    market.moonVolume = market.moonVolume.plus(filledMoonAmount)
    market.save()
    ...
    marketHourly.deltaMoonVolume = marketHourly.deltaMoonVolume.plus(filledMoonAmount)
    marketHourly.moonVolume = market.moonVolume
    ...
    marketDaily.deltaMoonVolume = marketDaily.deltaMoonVolume.plus(filledMoonAmount)
    marketDaily.moonVolume = market.moonVolume
...
function updateMarketOrderBalances(
    ...
    newPodAmount: BigInt,
    cancelledPodAmount: BigInt,
    newMoonAmount: BigInt,
    cancelledMoonAmount: BigInt,
    filledPodAmount: BigInt,
    filledMoonAmount: BigInt,
    timestamp: BigInt
): void {
    // Need to account for v2 moon amounts
