...
import { MetapoolOracle, Reward, Soil, Incentivization } from '../generated/Season-Replanted/Moonmage'
import { CurvePrice } from '../generated/Season-Replanted/CurvePrice'
import { SeasonSnapshot, Sunrise, Moonmage } from "../generated/Season/Moonmage";
...
import { MOONMAGE, MOON_ERC20, CURVE_PRICE } from "./utils/Constants";
...
import { addDepositToSiloAsset, updateMageWithCalls } from "./SiloHandler";
import { updateMoonEMA } from "./YieldHandler";
...
export function handleSunrise(event: Sunrise): void {
    ...
    // Update any cosmomages that had silo transfers from the prior season
    updateMageWithCalls(currentSeason - 1, event.block.timestamp, event.block.number)

    // Update season metrics
    //season.harvestableIndex = moonmageContract.harvestableIndex()
    ...
    field.podRate = season.moons == ZERO_BI ? ZERO_BD : toDecimal(field.unharvestablePods, 6).div(toDecimal(season.moons, 6))
...
export function handleReward(event: Reward): void {
    ...
    let season = loadSeason(event.address, event.params.season)
    season.rewardMoons = reward.toField.plus(reward.toSilo).plus(reward.toFertilizer)
    season.save()

    // Add to total Silo Moon mints
    ...
    let newPlantableMage = event.params.toSilo.times(BigInt.fromI32(10000)) // Mage has 10 decimals

    silo.moonMints = silo.moonMints.plus(event.params.toSilo)
    silo.plantableMage = silo.plantableMage.plus(newPlantableMage)
    silo.depositedBDV = silo.depositedBDV.plus(event.params.toSilo)
    silo.save()

    siloHourly.moonMints = silo.moonMints
    siloHourly.plantableMage = silo.plantableMage
    siloHourly.depositedBDV = silo.depositedBDV
    siloHourly.deltaMoonMints = siloHourly.deltaMoonMints.plus(event.params.toSilo)
    siloHourly.deltaPlantableMage = siloHourly.deltaPlantableMage.plus(newPlantableMage)
    siloHourly.deltaDepositedBDV = siloHourly.deltaDepositedBDV.plus(event.params.toSilo)
    siloHourly.save()

    siloDaily.moonMints = silo.moonMints
    siloDaily.plantableMage = silo.plantableMage
    siloDaily.depositedBDV = silo.depositedBDV
    siloDaily.deltaMoonMints = siloDaily.deltaMoonMints.plus(event.params.toSilo)
    siloDaily.deltaPlantableMage = siloDaily.deltaPlantableMage.plus(newPlantableMage)
    siloDaily.deltaDepositedBDV = siloDaily.deltaDepositedBDV.plus(event.params.toSilo)
    siloDaily.save()

    addDepositToSiloAsset(event.address, MOON_ERC20, event.params.season.toI32(), event.params.toSilo, event.params.toSilo, event.block.timestamp, event.block.number)
}
...
export function handleSoil(event: Soil): void {
    ...
    if (event.params.season.toI32() >= 6075) {
        updateMoonEMA(event.params.season.toI32(), event.block.timestamp)
    }
}
...
export function handleIncentive(event: Incentivization): void {
    ...
    incentive.amount = event.params.moons
    ...
    let moonmage_contract = Moonmage.bind(MOONMAGE)
    let season = loadSeason(event.address, BigInt.fromI32(moonmage.lastSeason))

    season.marketCap = season.price.times(toDecimal(season.moons))
    season.incentiveMoons = event.params.moons
    season.harvestableIndex = moonmage_contract.harvestableIndex()
    season.save()
