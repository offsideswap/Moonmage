...
function handleTransfer(from: Address, to: Address, id: BigInt, amount: BigInt, blockNumber: BigInt): void {
    ...
    if (from != ADDRESS_ZERO) {
        let fromCosmonaut = loadCosmonaut(from)
        let fromFertilizerBalance = loadFertilizerBalance(fertilizerToken, fromCosmonaut)
    ...
    let toCosmonaut = loadCosmonaut(to)
    let toFertilizerBalance = loadFertilizerBalance(fertilizerToken, toCosmonaut)
