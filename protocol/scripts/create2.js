...
function create2Address(name, symbol, searchingFor, start = 0) {
    ...
    const format = `./artifacts/contracts/tokens/ERC20/MoonmageERC20.sol/MoonmageERC20.json`
    const threePoolJson = fs.readFileSync(format);
    const bytecode = JSON.parse(threePoolJson).bytecode

    const encoded = ethers.utils.defaultAbiCoder.encode([ "address", "string", "string" ], [ MOONMAGE, name, symbol ]);
    ...
    for(let salt = start; salt <= 1000000000000; salt++) {
        ...
        const result = ethers.utils.getCreate2Address(MOONMAGE, saltHex, ethers.utils.keccak256(yay));
