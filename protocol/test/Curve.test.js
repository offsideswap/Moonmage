...
const { MOON, THREE_CURVE, THREE_POOL, MOON_3_CURVE, STABLE_FACTORY, USDC } = require('./utils/constants')
...
describe('Curve', function () {
  before(async function () {
    ...
    await this.moon.connect(user).approve(this.diamond.address, '100000000000')
    await this.moon.connect(user2).approve(this.diamond.address, '100000000000') 
    ...
    await this.moonMetapool.connect(user).approve(this.diamond.address, to18('100000000000'))
  ...
  describe("Add Liquidity", async function () {
    describe("To external", async function () {
      ...
      it('updates moonmage balances', async function () {
        expect(await this.moon.balanceOf(this.token.address)).to.be.eq('0');
        expect(await this.threeCurve.balanceOf(this.token.address)).to.be.eq('0');
        expect(await this.moonMetapool.balanceOf(this.token.address)).to.be.eq('0');
      })

      it('adds lp tokens to external balance', async function () {
        expect(await this.token.getExternalBalance(userAddress, this.moonMetapool.address)).to.be.equal(to18('2000'))
      })

      it('adds lp tokens to internal balance', async function () {
        expect(await this.token.getInternalBalance(userAddress, this.moonMetapool.address)).to.be.equal(to18('0'))
      })
    })
    ...
    describe("To internal", async function () {
      ...
      it('updates moonmage balances', async function () {
        expect(await this.moon.balanceOf(this.token.address)).to.be.eq('0');
        expect(await this.threeCurve.balanceOf(this.token.address)).to.be.eq('0');
        expect(await this.moonMetapool.balanceOf(this.token.address)).to.be.eq(to18('1000'));
      })
      ...
      it('adds lp tokens to internal balance', async function () {
        expect(await this.token.getInternalBalance(userAddress, this.moonMetapool.address)).to.be.equal(to18('1000'))
      })
    })
  })
  ...
  describe("Remove Liqudity", async function () {
    describe("To external", async function () {
      ...
      it('adds lp tokens to internal balance', async function () {
        ...
        expect(await this.token.getInternalBalance(userAddress, this.moon.address)).to.be.equal('0')
      })

      it('adds lp tokens to external balance', async function () {
        expect(await this.token.getExternalBalance(userAddress, this.threeCurve.address)).to.be.equal(to18('250'))
        expect(await this.token.getExternalBalance(userAddress, this.moon.address)).to.be.equal(to6('9250'))
      })
    })
    ...
    describe("To internal", async function () {
      ...
      it('updates moonmage balances', async function () {
        expect(await this.moon.balanceOf(this.token.address)).to.be.eq(to6('250'))
      ...
      it('adds lp tokens to internal balance', async function () {
        ...
        expect(await this.token.getInternalBalance(userAddress, this.moon.address)).to.be.equal(to6('250'))
      })

      it('adds lp tokens to external balance', async function () {
        expect(await this.token.getExternalBalance(userAddress, this.threeCurve.address)).to.be.equal(to18('0'))
        expect(await this.token.getExternalBalance(userAddress, this.moon.address)).to.be.equal(to6('9000'))
      })
    })
  })
  ...
  describe("Remove Liqudity Inbalance", async function () {
    describe("To external", async function () {
      ...
      it('adds lp tokens to external balance', async function () {
        ...
        expect(await this.token.getExternalBalance(userAddress, this.moon.address)).to.be.equal(to6('9400'))
      })
    })
    ...
    describe("To internal", async function () {
      ...
      it('updates moonmage balances', async function () {
        expect(await this.moon.balanceOf(this.token.address)).to.be.eq(to6('400'))
      ...
      it('adds lp tokens to internal balance', async function () {
        ...
        expect(await this.token.getInternalBalance(userAddress, this.moon.address)).to.be.equal(to6('400'))
      })
  ...
  describe("Remove Liqudity one token", async function () {
    describe("To external", async function () {
      ...
      it('adds lp tokens to external balance', async function () {
        ...
        expect(await this.token.getExternalBalance(userAddress, this.moon.address)).to.be.equal('9491960239')
      })
    })
    ...
    describe("To internal", async function () {
      ...
      it('updates moonmage balances', async function () {
        expect(await this.moon.balanceOf(this.token.address)).to.be.eq('491960239')
        expect(await this.threeCurve.balanceOf(this.token.address)).to.be.eq('0')
        expect(await this.moonMetapool.balanceOf(this.token.address)).to.be.eq('0')
      })
      ...
      it('adds lp tokens to internal balance', async function () {
        expect(await this.token.getInternalBalance(userAddress, this.threeCurve.address)).to.be.equal('0')
        expect(await this.token.getInternalBalance(userAddress, this.moon.address)).to.be.equal('491960239')
      })
  ...
  describe("farm LP and Deposit", async function () {
    beforeEach('add LP and Deposits', async function () {
      ...
      await this.moon.mint(user2Address, to6('500'))
      await this.threeCurve.connect(user2).approve(this.silo.address, to18('5000000'))
      await this.moon.connect(user2).approve(this.silo.address, to6('50000000'))
    ...
    it('add lp and deposit', async function () {
      ...
      const dep = await this.silo.getDeposit(user2Address, this.moonMetapool.address, season)
