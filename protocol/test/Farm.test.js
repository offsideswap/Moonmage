...
const { MOON, USDT, WETH, CURVE_REGISTRY, CRYPTO_REGISTRY, THREE_POOL, TRI_CRYPTO, TRI_CRYPTO_POOL, THREE_CURVE, MOON_3_CURVE, USDC, WBTC, DAI, LUSD_3_CURVE, LUSD, CRYPTO_FACTORY } = require('./utils/constants')
...
describe('Farm', function () {
  if (!!process.env.FORKING_RPC) {
    before(async function () {
      ...
      await this.moonMetapool.set_virtual_price(to18('1'))
      await this.moonMetapool.set_balances(['1', ethers.utils.parseUnits('1', 12)])
      await this.moonMetapool.set_supply('0')

      await this.season.lightSunrise();
      await this.moon.connect(user).mint(user.address, '100000000000');
    ...
    describe('Farm Deposit', function () {
      before(async function () {
        deposit = await this.silo.interface.encodeFunctionData(
          "deposit", [this.moon.address, to6('1'), EXTERNAL]
        );
      })
    ...
    //   addLPMoonMetapool = await this.curve.interface.encodeFunctionData('addLiquidity', [
    //     MOON_3_CURVE, // 3pool
    //     STABLE_FACTORY,
    //     [ to6('1'), to18('1') ],
    //     to18('2'),  // minAmountOut
    ...
    //   deposit = await this.silo.interface.encodeFunctionData(
    //     "deposit", [MOON_3_CURVE, to18('2'), INTERNAL]
    //   );

    //   withdraw = await this.silo.interface.encodeFunctionData(
    //     "withdrawDeposit", [MOON_3_CURVE, 2, to18('2')]
    //   );
