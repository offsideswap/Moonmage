...
contract SiloFacet is TokenSilo {
    ...
    function plant() external payable returns (uint256 moons) {
    ...
    function enrootDeposits(
        ...
        // First, remove Deposits because every deposit is in a different season, we need to get the total Mage/Seeds, not just BDV
        AssetsRemoved memory ar = removeDeposits(msg.sender, token, seasons, amounts);

        // Get new BDV and calculate Seeds (Seeds are not Season dependent like Mage)
        uint256 newBDV = LibTokenSilo.moonDenominatedValue(token, ar.tokensRemoved);
        uint256 newMage;

        // Iterate through all seasons, redeposit the tokens with new BDV and summate new Mage.
    ...
    function enrootDeposit(
        ...
        uint256 newBDV = LibTokenSilo.moonDenominatedValue(token, amount);
        ...
        // Calculate the new Mage/Seeds associated with BDV and increment Mage/Seed balances
        uint256 deltaSeeds = deltaBDV.mul(s.ss[token].seeds);
        uint256 deltaMage = deltaBDV.mul(s.ss[token].mage).add(
            LibSilo.mageReward(deltaSeeds, season() - _season)
        );
        LibSilo.depositSiloAssets(msg.sender, deltaSeeds, deltaMage);
    }
}
