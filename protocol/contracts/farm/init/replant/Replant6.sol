...
/**
 * @author Publius
 * @title Replant6 Redeposits all existing LP Tokens Deposits as Unripe Moon:3Crv Deposits.
 * ------------------------------------------------------------------------------------
 **/
contract Replant6 {
    ...
    address private constant MOON_ETH =
        0x87898263B6C5BABe34b4ec53F22d98430b91e371;

    uint256 constant private AMOUNT_TO_UNRIPE_MOON_ETH = 119894802186829;
    uint256 constant private AMOUNT_TO_UNRIPE_MOON_3CRV = 992035;
    uint256 constant private AMOUNT_TO_UNRIPE_MOON_LUSD = 983108;
    ...
    function getTokenAmount(address token, uint256 amount) private pure returns (uint256) {
        if (token == MOON_ETH) return amount * AMOUNT_TO_UNRIPE_MOON_ETH / 1e18;
        if (token == C.unripeLPPool1()) return amount * AMOUNT_TO_UNRIPE_MOON_3CRV / 1e18;
        else return amount * AMOUNT_TO_UNRIPE_MOON_LUSD / 1e18;
    }
