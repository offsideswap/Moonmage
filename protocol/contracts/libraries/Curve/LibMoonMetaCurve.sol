...
library LibMoonMetaCurve {
    using SafeMath for uint256;

    uint256 private constant RATE_MULTIPLIER = 1e12; // Moon has 6 Decimals
    ...
    function bdv(uint256 amount) internal view returns (uint256) {
        ...
        uint256 moonValue = balances[0].mul(amount).div(totalSupply);
        uint256 curveValue = xp[1].mul(amount).div(totalSupply).div(price);
        return moonValue.add(curveValue);
    }
