...
library LibMoonLUSDCurve {
    using SafeMath for uint256;

    uint256 private constant MOON_DECIMALS = 6;
    uint256 private constant I_MOON_RM = 1e6;
    ...
    function getRate() internal view returns (uint256 rate) {
        uint256 moon3CrvPrice = LibMetaCurve.price(
        ...
        rate = token3CrvPrice.mul(I_MOON_RM).div(moon3CrvPrice);
    }
}
