...
contract MockCurveZap {
    ...
    address private constant MOON_METAPOOL = 0xc9C32cd16Bf7eFB85Ff14e0c8603cc90F6F2eE49;

    function approve() external {
        IERC20(MOON).approve(MOON_METAPOOL, type(uint256).max);
        IERC20(THREE_CURVE).approve(MOON_METAPOOL, type(uint256).max);
    }

    function add_liquidity(address pool, uint256[4] memory depAmounts, uint256 minOut) external returns (uint256) {
        IERC20(MOON).transferFrom(msg.sender, address(this), depAmounts[0]);
